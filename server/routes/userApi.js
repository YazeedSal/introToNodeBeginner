...
import authByUsernameAndPassword from "../../services/authByEmailAndPassword.js";
...
router.post("/register", async function (req, res) {
  try {
    ...
    const doesExist = await checkUserAvailability(signUpForm);
    if (doesExist.email)
...
router.put("/signin", async function (req, res) {
  try {
    ...
    const dbRes = await authByUsernameAndPassword(username, password);
    res.send(dbRes);
  } catch (error) {
...
router.delete("/deactivate", auth, async function (req, res) {
  try {
    const { user } = req;
    user.isActive = false;
    await user.save();
